...
import { bomData } from '../dummy-data/bomData';
...
export const BomManagementPage: React.FC = () => {
			...
			cell: ({
				...
				return (
					<Link
						...
						to={`/aips/master-data/bom-management/${id}`}
					>
						{value || '-'}
					</Link>
				);
	...
	const dummyRemove = {
		mutate: (id: number) => {
			console.log('Delete BOM with id:', id);
	...
	const handleEdit = () => {
		if (editItemData) {
			...
			toast.warning('수정하실 BOM을 선택해주세요.');
		}
	};
	...
	const handleDelete = () => {
		if (editItemData) {
			...
			toast.warning('삭제하실 BOM을 선택해주세요.');
		}
	};
	...
	useEffect(() => {
		setData(bomData);
		setTotalElements(bomData.length);
		setPageCount(Math.ceil(bomData.length / DEFAULT_PAGE_SIZE));
	}, []);
	...
	return (
		<>
			<DraggableDialog
				...
				title={editItemData ? 'BOM 수정' : 'BOM 등록'}
				content={
					<div className="p-6">
			...
			<DeleteConfirmDialog
				...
				title="BOM 삭제"
				description={`선택한 BOM '${editItemData?.itemName || editItemData?.itemNumber}'을(를) 삭제하시겠습니까? 이 작업은 취소할 수 없습니다.`}
			/>
