...
import IniBomListPage from '@aips/pages/ini/bom/IniBomListPage';
import IniBomRegisterPage from '@aips/pages/ini/bom/IniBomRegisterPage';
...
const BomTabNavigation: React.FC<TabNavigationProps> = ({ activetab }) => {
	...
	useEffect(() => {
		...
		if (pathname.includes('/ini/bom/list')) {
			setCurrentTab('list');
		}
	}, [location.pathname]);
	...
	return (
		<>
			<DraggableDialog
				...
				title={`${t('tabs.titles.bomManagement')} ${t('tabs.actions.register')}`}
				content={
					<IniBomRegisterPage onClose={() => setOpenModal(false)} />
				}
			/>
			<TabLayout
				title={t('tabs.titles.bomManagement')}
				tabs={tabs}
