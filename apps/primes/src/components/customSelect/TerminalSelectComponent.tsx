...
import { useTerminalFieldQuery } from '@primes/hooks/init/terminal/useTerminalFieldQuery';
...
export const TerminalSelectComponent: React.FC<TerminalSelectProps> = ({
	fieldKey = 'terminalName',
	valueKey = 'id',
	labelKey = 'value',
	placeholder,
	onChange,
	...
	const { data, isLoading, error } = useTerminalFieldQuery(fieldKey);
	...
	const defaultPlaceholder = placeholder || '터미널을 선택하세요';
		...
		return (
			<div className={className}>
				<ComboBox
					...
					value={null}
					onChange={() => {}}
					placeholder="터미널 조회 중 오류가 발생했습니다"
					disabled={true}
				/>
			</div>
		);
