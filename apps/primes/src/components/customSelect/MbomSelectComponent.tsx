...
import { useMbomFieldQuery } from '@primes/hooks/init/mbom/useMbomFieldQuery';

interface MbomSelectProps {
	fieldKey?: string;
	valueKey?: string;
	labelKey?: string;
	value?: string | null;
...
export const MbomSelectComponent: React.FC<MbomSelectProps> = ({
	fieldKey = 'mbomName',
	valueKey = 'id',
	labelKey = 'value',
	placeholder,
	...
	const { data, isLoading, error } = useMbomFieldQuery(fieldKey);
	...
	const defaultPlaceholder = placeholder || 'M-BOM을 선택하세요';
		...
		return (
			<div className={className}>
				<ComboBox
					...
					value={null}
					onChange={() => {}}
					placeholder="M-BOM 조회 중 오류가 발생했습니다"
					disabled={true}
				/>
			</div>
		);
