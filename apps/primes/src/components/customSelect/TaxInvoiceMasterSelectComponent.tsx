...
import { useTaxInvoiceMasterFieldQuery } from '@primes/hooks/sales/taxInvoiceMaster/useTaxInvoiceMasterFieldQuery';

interface TaxInvoiceMasterSelectProps {
	fieldKey?: string;
	valueKey?: string;
	labelKey?: string;
	value?: string | null;
...
export const TaxInvoiceMasterSelectComponent: React.FC<
	TaxInvoiceMasterSelectProps
> = ({
	fieldKey = 'taxInvoiceCode',
	valueKey = 'id',
	labelKey = 'value',
	placeholder,
	onChange,
	...
	const { data, isLoading, error } = useTaxInvoiceMasterFieldQuery(fieldKey);
	...
	const defaultPlaceholder = placeholder || '세금계산서를 선택하세요';
		...
		return (
			<div className={className}>
				<ComboBox
					...
					value={null}
					onChange={() => {}}
					placeholder="세금계산서 조회 중 오류가 발생했습니다"
					disabled={true}
				/>
			</div>
		);
