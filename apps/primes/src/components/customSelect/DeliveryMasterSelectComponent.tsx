...
import { useDeliveryMasterFieldQuery } from '@primes/hooks/sales/deliveryMaster/useDeliveryMasterFieldQuery';

interface DeliveryMasterSelectProps {
	fieldKey?: string;
	valueKey?: string;
	labelKey?: string;
	value?: string | null;
...
export const DeliveryMasterSelectComponent: React.FC<
	DeliveryMasterSelectProps
> = ({
	fieldKey = 'deliveryCode',
	valueKey = 'id',
	labelKey = 'value',
	placeholder,
	onChange,
	...
	const { data, isLoading, error } = useDeliveryMasterFieldQuery(fieldKey);
	...
	const defaultPlaceholder = placeholder || '납품을 선택하세요';
		...
		return (
			<div className={className}>
				<ComboBox
					...
					value={null}
					onChange={() => {}}
					placeholder="납품 조회 중 오류가 발생했습니다"
					disabled={true}
				/>
			</div>
		);
