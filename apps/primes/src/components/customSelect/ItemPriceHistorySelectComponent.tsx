...
import { useItemPriceHistoryFieldQuery } from '@primes/hooks/purchase/itemPriceHistory/useItemPriceHistoryFieldQuery';

interface ItemPriceHistorySelectProps {
	fieldKey?: string;
	valueKey?: string;
	labelKey?: string;
	value?: string | null;
...
export const ItemPriceHistorySelectComponent: React.FC<
	ItemPriceHistorySelectProps
> = ({
	fieldKey = 'itemName',
	valueKey = 'id',
	labelKey = 'value',
	...
	const { data, isLoading, error } = useItemPriceHistoryFieldQuery(fieldKey);
	...
	const defaultPlaceholder = placeholder || '품목가격이력을 선택하세요';
		...
		return (
			<div className={className}>
				<ComboBox
					...
					value={null}
					onChange={() => {}}
					placeholder="품목가격이력 조회 중 오류가 발생했습니다"
					disabled={true}
				/>
			</div>
		);
