...
import { useItemsVendorFieldQuery } from '@primes/hooks/purchase/itemsVendor/useItemsVendorFieldQuery';

interface ItemsVendorSelectProps {
	fieldKey?: string;
	valueKey?: string;
	labelKey?: string;
	value?: string | null;
...
export const ItemsVendorSelectComponent: React.FC<ItemsVendorSelectProps> = ({
	fieldKey = 'vendorName',
	valueKey = 'id',
	labelKey = 'value',
	placeholder,
	...
	const { data, isLoading, error } = useItemsVendorFieldQuery(fieldKey);
	...
	const defaultPlaceholder = placeholder || '품목거래처를 선택하세요';
		...
		return (
			<div className={className}>
				<ComboBox
					...
					value={null}
					onChange={() => {}}
					placeholder="품목거래처 조회 중 오류가 발생했습니다"
					disabled={true}
				/>
			</div>
		);
