...
import { useEstimateMasterFieldQuery } from '@primes/hooks/sales/estimateMaster/useEstimateMasterFieldQuery';

interface EstimateMasterSelectProps {
	fieldKey?: string;
	valueKey?: string;
	labelKey?: string;
	value?: string | null;
...
export const EstimateMasterSelectComponent: React.FC<
	EstimateMasterSelectProps
> = ({
	fieldKey = 'estimateCode',
	valueKey = 'id',
	labelKey = 'value',
	placeholder,
	onChange,
	...
	const { data, isLoading, error } = useEstimateMasterFieldQuery(fieldKey);
	...
	const defaultPlaceholder = placeholder || '견적을 선택하세요';
		...
		return (
			<div className={className}>
				<ComboBox
					...
					value={null}
					onChange={() => {}}
					placeholder="견적 조회 중 오류가 발생했습니다"
					disabled={true}
				/>
			</div>
		);
