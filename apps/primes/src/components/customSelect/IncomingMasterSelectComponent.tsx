...
interface IncomingMasterSelectProps {
	fieldKey?: string;
	valueKey?: string;
	labelKey?: string;
	value?: string | null;
...
export const IncomingMasterSelectComponent: React.FC<
	IncomingMasterSelectProps
> = ({
	fieldKey = 'incomingCode',
	valueKey = 'id',
	labelKey = 'value',
	placeholder,
	onChange,
	...
	const { data, isLoading, error } = useIncomingMasterFieldQuery(fieldKey);
	...
	const defaultPlaceholder = placeholder || '입고를 선택하세요';
		...
		return (
			<div className={className}>
				<ComboBox
					...
					value={null}
					onChange={() => {}}
					placeholder="입고 조회 중 오류가 발생했습니다"
					disabled={true}
				/>
			</div>
		);
