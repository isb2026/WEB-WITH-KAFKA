...
import { useProgressRouteFieldQuery } from '@primes/hooks/init/progressRoute/useProgressRouteFieldQuery';

interface ProgressRouteSelectProps {
	fieldKey?: string;
	valueKey?: string;
	labelKey?: string;
	value?: string | null;
...
export const ProgressRouteSelectComponent: React.FC<
	ProgressRouteSelectProps
> = ({
	...
	const { data, isLoading, error } = useProgressRouteFieldQuery(fieldKey);
	...
	const defaultPlaceholder = placeholder || '공정경로를 선택하세요';
		...
		return (
			<div className={className}>
				<ComboBox
					...
					value={null}
					onChange={() => {}}
					placeholder="공정경로 조회 중 오류가 발생했습니다"
					disabled={true}
				/>
			</div>
		);
