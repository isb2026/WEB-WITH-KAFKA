...
import { useCheckingSpecFieldQuery } from '@primes/hooks/qms/checkingSpec/useCheckingSpecFieldQuery';

interface CheckingSpecSelectProps {
	fieldKey?: string;
	valueKey?: string;
	labelKey?: string;
	value?: string | null;
...
export const CheckingSpecSelectComponent: React.FC<CheckingSpecSelectProps> = ({
	fieldKey = 'checkingName',
	valueKey = 'id',
	labelKey = 'value',
	placeholder,
	onChange,
	...
	const { data, isLoading, error } = useCheckingSpecFieldQuery(fieldKey);
	...
	const defaultPlaceholder = placeholder || '검사규격을 선택하세요';
		...
		return (
			<div className={className}>
				<ComboBox
					...
					value={null}
					onChange={() => {}}
					placeholder="검사규격 조회 중 오류가 발생했습니다"
					disabled={true}
				/>
			</div>
		);
