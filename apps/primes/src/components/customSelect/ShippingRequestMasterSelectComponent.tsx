...
import { useShippingRequestMasterFieldQuery } from '@primes/hooks/sales/shippingRequestMaster/useShippingRequestMasterFieldQuery';

interface ShippingRequestMasterSelectProps {
	fieldKey?: string;
	valueKey?: string;
	labelKey?: string;
	value?: string | null;
...
export const ShippingRequestMasterSelectComponent: React.FC<
	ShippingRequestMasterSelectProps
> = ({
	fieldKey = 'shippingRequestCode',
	valueKey = 'id',
	labelKey = 'value',
	placeholder,
	onChange,
	...
	const { data, isLoading, error } =
		useShippingRequestMasterFieldQuery(fieldKey);
	...
	const defaultPlaceholder = placeholder || '출하요청을 선택하세요';
		...
		return (
			<div className={className}>
				<ComboBox
					...
					value={null}
					onChange={() => {}}
					placeholder="출하요청 조회 중 오류가 발생했습니다"
					disabled={true}
				/>
			</div>
		);
