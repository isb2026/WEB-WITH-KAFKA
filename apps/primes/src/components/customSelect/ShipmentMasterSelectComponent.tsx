...
import { useShipmentMasterFieldQuery } from '@primes/hooks/sales/shipmentMaster/useShipmentMasterFieldQuery';

interface ShipmentMasterSelectProps {
	fieldKey?: string;
	valueKey?: string;
	labelKey?: string;
	value?: string | null;
...
export const ShipmentMasterSelectComponent: React.FC<
	ShipmentMasterSelectProps
> = ({
	fieldKey = 'shipmentCode',
	valueKey = 'id',
	labelKey = 'value',
	placeholder,
	onChange,
	...
	const { data, isLoading, error } = useShipmentMasterFieldQuery(fieldKey);
	...
	const defaultPlaceholder = placeholder || '출하를 선택하세요';
		...
		return (
			<div className={className}>
				<ComboBox
					...
					value={null}
					onChange={() => {}}
					placeholder="출하 조회 중 오류가 발생했습니다"
					disabled={true}
				/>
			</div>
		);
