...
import { usePurchaseMasterFieldQuery } from '@primes/hooks/purchase/purchaseMaster/usePurchaseMasterFieldQuery';

interface PurchaseMasterSelectProps {
	fieldKey?: string;
	valueKey?: string;
	labelKey?: string;
	value?: string | null;
...
export const PurchaseMasterSelectComponent: React.FC<
	PurchaseMasterSelectProps
> = ({
	fieldKey = 'purchaseCode',
	valueKey = 'id',
	labelKey = 'value',
	placeholder,
	onChange,
	...
	const { data, isLoading, error } = usePurchaseMasterFieldQuery(fieldKey);
	...
	const defaultPlaceholder = placeholder || '구매를 선택하세요';
		...
		return (
			<div className={className}>
				<ComboBox
					...
					value={null}
					onChange={() => {}}
					placeholder="구매 조회 중 오류가 발생했습니다"
					disabled={true}
				/>
			</div>
		);
