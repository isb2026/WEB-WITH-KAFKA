...
import { useMachineCheckSpecFieldQuery } from '@primes/hooks/machine/useMachineCheckSpecFieldQuery';

interface MachineCheckSpecSelectProps {
	fieldKey?: string;
	valueKey?: string;
	labelKey?: string;
	value?: string | null;
...
export const MachineCheckSpecSelectComponent: React.FC<
	MachineCheckSpecSelectProps
> = ({
	fieldKey = 'checkSpecName',
	valueKey = 'id',
	labelKey = 'value',
	placeholder,
	onChange,
	...
	const { data, isLoading, error } = useMachineCheckSpecFieldQuery(fieldKey);
	...
	const defaultPlaceholder = placeholder || '설비점검규격을 선택하세요';
		...
		return (
			<div className={className}>
				<ComboBox
					...
					value={null}
					onChange={() => {}}
					placeholder="설비점검규격 조회 중 오류가 발생했습니다"
					disabled={true}
				/>
			</div>
		);
