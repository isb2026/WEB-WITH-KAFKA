...
import { usePlanFieldQuery } from '@primes/hooks/production/plan/usePlanFieldQuery';

interface ProductionPlanSelectProps {
	fieldKey?: string;
	valueKey?: string;
	labelKey?: string;
	value?: string | null;
...
export const ProductionPlanSelectComponent: React.FC<
	ProductionPlanSelectProps
> = ({
	fieldKey = 'planName',
	valueKey = 'id',
	labelKey = 'value',
	...
	const { data, isLoading, error } = usePlanFieldQuery(fieldKey);
	...
	const defaultPlaceholder = placeholder || '생산계획을 선택하세요';
		...
		return (
			<div className={className}>
				<ComboBox
					...
					value={null}
					onChange={() => {}}
					placeholder="생산계획 조회 중 오류가 발생했습니다"
					disabled={true}
				/>
			</div>
		);
