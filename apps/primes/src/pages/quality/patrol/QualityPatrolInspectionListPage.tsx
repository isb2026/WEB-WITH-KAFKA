...
export const QualityPatrolInspectionListPage: React.FC = () => {
	...
	const [searchParams, setSearchParams] = useState<any>({inspectionType: 'PATROL'});
	...
	const tableColumns = [
		...
		},
		{
			...
			header: tDataTable('columns.progressName'),
			size: 150,
			cell: ({ row }: { row: any }) => {
				const meta = row.original.meta;
				const progressName = meta?.progressName || '-';
		...
		},
		{
			...
			cell: ({ row }: { row: any }) => {
				...
				const handleCodeClick = () => {
					navigate(
						`/quality/patrol-inspection/report/${row.original.id}`
					);
				};
		...
		},
		{
			...
			size: 100,
			cell: tCommon('inspection.type.patrol'),
		},
		{
			id: 'inspectorName',
	...
	const handleEdit = () => {
		if (selectedInspection) {
			navigate(`/quality/patrol-inspection/${selectedInspection.id}`);
		}
	};
	...
	const handleDelete = () => {
		if (selectedRows) {
			...
			console.log('No inspection selected');
			toast.error('삭제할 검사 항목을 선택해주세요.');
		}
	};

	const handleDeleteConfirm = async () => {
		// selectedRows의 인덱스를 사용하여 실제 데이터의 아이디를 추출
		const selectedInspectionIds = Array.from(selectedRows).map(index => {
			// 현재 페이지의 데이터에서 해당 인덱스의 아이디를 가져옴
			const currentPageData = list.data?.content || [];
			return currentPageData[index]?.id;
		...
		<>
			...
			<PageTemplate>
				<DatatableComponent
					...
					data={data}
					tableTitle={tCommon('inspection.type.patrol')}
...
export default QualityPatrolInspectionListPage;
