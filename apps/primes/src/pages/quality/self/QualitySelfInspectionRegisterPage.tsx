...
import { CheckingSpecData, CheckingSpecSearchRequest } from '@primes/types/qms/checkingSpec';
...
const initializeMeasuredValues = (sampleQuantity: number = 1): { [key: string]: number } => {
...
// 메인 컴포넌트
// ============================================================================
const QualitySELFInspectionRegisterPage: React.FC = (): JSX.Element => {
	// ========================================================================
	// 기본 훅 및 번역
	// ========================================================================
	...
	const location = useLocation(); // ✅ 추가: 현재 위치 정보 가져오기
	...
	const [allCheckingSpecs, setAllCheckingSpecs] = useState<CheckingSpecData[]>([]); // 모든 검사 스펙 저장
	...
	// API 훅 및 요청 상태
	// ========================================================================
	const [itemProgressSearchRequest, setItemProgressSearchRequest] = useState<ItemProgressSearchRequest | null>(null);
	const { list: itemProgressList } = useProgress({
		searchRequest: itemProgressSearchRequest || { itemId: 0 },
		page: DEFAULT_PAGE,
		size: DEFAULT_PAGE_SIZE,
		enabled: itemProgressSearchRequest !== null, // ✅ 초기 렌더링 시 API 호출 방지
	});

	const [checkingSpecSearchRequest, setCheckingSpecSearchRequest] = useState<CheckingSpecSearchRequest | null>(null);
	const { data: checkingSpecs, refetch: refetchCheckingSpecs } = useCheckingSpecListQuery({
		searchRequest: checkingSpecSearchRequest || { targetId: 0, inspectionType: INSPECTION_TYPE },
	});
	...
	// 이벤트 핸들러 - 데이터 조회
	// ========================================================================
	const handleItemSearch = useCallback(async (data: any): Promise<void> => {
		...
		try {
			...
			// ✅ CommandSearchInput에서 이미 공정 데이터를 가져온 경우 우선 사용
			if (data.progressData && Array.isArray(data.progressData) && data.progressData.length > 0) {
				progressData = data.progressData;
			} else {
				// ✅ 공정 데이터가 없는 경우에만 별도 API 호출
				...
				while (retryCount < maxRetries) {
					...
					// API 호출이 성공한 경우
					if (itemProgressListData.isSuccess) {
						const responseData = itemProgressListData.data;
						...
						// content가 배열이면 사용하고, 없으면 빈 배열
						progressData = Array.isArray(responseData.content) 
			...
			if (progressData.length === 0) {
				toast.warning(t('pages.messages.noProgressForItem'));
			}

			const progressOptions = progressData
				.sort((a: any, b: any) => (a.progressOrder || 0) - (b.progressOrder || 0))
				.map((progress: any) => ({
					value: progress.id.toString(),
					label: progress.progressName || progress.progressTypeName || `${t('tabs.labels.progress')} ${progress.progressOrder}`,
					progressId: progress.id
				}));

			setMasterData((prev) => ({
				...prev,
				workOrderNo: data.commandNo || '',
	...
	const handleDataNotFound = useCallback(() => {
	...
	const handleProgressChange = useCallback(async (progressId: string) => {
		...
		try {
			...
			const searchRequest = {
				...
				inspectionTypes: ['SELF', 'PATROL'], // SELF와 PATROL 타입 모두 서버에서 가져옴
				isDelete: false,
				isUse: true
			};
			
			// 상태 업데이트 후 약간의 지연을 두고 API 호출
			setCheckingSpecSearchRequest(searchRequest);
			
			// 상태가 업데이트될 시간을 주기 위해 약간 대기
			await new Promise(resolve => setTimeout(resolve, 100));
			
			// API 응답을 기다림
			const checkingSpecsResponse = await refetchCheckingSpecs();
			//refetchCheckingSpecs 호출 후 약간 대기 후에 데이터 가져오기
			await new Promise(resolve => setTimeout(resolve, 500));
			const specs = checkingSpecsResponse.data?.content || [];
			
			// 모든 스펙을 로컬 상태에 저장
			setAllCheckingSpecs(specs);
			
			// API 응답이 완료된 후에만 알림 처리
			if (specs.length === 0) {
				// 서버에서 가져온 전체 스펙이 없는 경우
				setInspectionItems([]);
				toast.warning('선택한 공정에 등록된 검사 스펙이 없습니다.');
				return;
			}
			
			// 현재 선택된 inspectionType에 따라 필터링하여 표시
			const currentInspectionType = formMethodsRef.current?.getValues('inspectionType') || masterData.inspectionType || 'SELF';
			
			let filteredSpecs = specs;
			if (currentInspectionType === 'SELF') {
				// SELF 선택 시: SELF 타입만 표시
				filteredSpecs = specs.filter((spec: CheckingSpecData) => spec.inspectionType === 'SELF');
			} else if (currentInspectionType === 'PATROL') {
				// PATROL 선택 시: SELF와 PATROL 모두 표시
				filteredSpecs = specs.filter((spec: CheckingSpecData) => 
					spec.inspectionType === 'SELF' || spec.inspectionType === 'PATROL'
				);
			} else {
				// 검사 유형이 선택되지 않은 경우 빈 배열
				filteredSpecs = [];
			}
			
			if (filteredSpecs.length > 0) {
				const newInspectionItems: InspectionItem[] = filteredSpecs.map((spec: CheckingSpecData, index: number) => ({
					id: index + 1,
					...
					measuredValues: initializeMeasuredValues(spec.sampleQuantity),
					specType: spec.specType?.toString() || '',
				...
				setInspectionItems([]);
				// 필터링된 결과가 없는 경우에만 알림 (검사 유형이 선택된 경우)
				if (currentInspectionType) {
					toast.warning(`선택한 검사유형(${currentInspectionType})에 해당하는 검사 스펙이 없습니다.`);
				}
			}
		} catch (error) {
			toast.error(t('inspection.toast.loadError'));
	...
	}, [refetchCheckingSpecs, t, masterData.inspectionType]);

	// inspectionType 변경 이벤트 핸들러
	const handleInspectionTypeChange = useCallback((inspectionType: string) => {
		if (!inspectionType?.trim()) return;
		
		// masterData의 inspectionType 업데이트
		setMasterData(prev => ({
			...prev,
			inspectionType: inspectionType
		}));
		
		// 로컬에 저장된 모든 검사 스펙을 기반으로 필터링
		if (allCheckingSpecs.length > 0) {
			let filteredSpecs = allCheckingSpecs;
			
			if (inspectionType === 'SELF') {
				// SELF 선택 시: SELF 타입만 표시
				filteredSpecs = allCheckingSpecs.filter((spec: CheckingSpecData) => spec.inspectionType === 'SELF');
			} else if (inspectionType === 'PATROL') {
				// PATROL 선택 시: SELF와 PATROL 모두 표시
				filteredSpecs = allCheckingSpecs.filter((spec: CheckingSpecData) => 
					spec.inspectionType === 'SELF' || spec.inspectionType === 'PATROL'
				);
			} else {
				// 기타 경우 빈 배열
			...
			if (filteredSpecs.length > 0) {
				...
				setInspectionItems(newInspectionItems);
				toast.success(`${inspectionType} 검사유형의 검사항목을 불러왔습니다. (${newInspectionItems.length}개)`);
			} else {
				setInspectionItems([]);
				// 필터링된 결과가 없는 경우 알림 (handleProgressChange에서 이미 처리되므로 제거)
			}
		} else {
			// 로컬에 저장된 스펙이 없으면 검사 항목 초기화 (알림 없음)
			setInspectionItems([]);
		}
	}, [allCheckingSpecs, t]);
		...
		const updatedItems = inspectionItems.map((item) => {
			...
			for (let i = 1; i <= sampleQuantity; i++) {
				...
				if (item.specType === 'TOLERANCE') {
					const tolerance = parseFloat(item.meta?.tolerance?.toString() || '0');
					if (rand < 0.6) {
						sampleValue = baseValue + (Math.random() - 0.5) * tolerance * 0.7;
					} else if (rand < 0.85) {
						sampleValue = baseValue + (Math.random() - 0.5) * tolerance * 0.9;
					} else {
						sampleValue = baseValue + (Math.random() - 0.5) * tolerance * 1.2;
					}
					...
					if (item.meta?.maxValue) {
						...
						if (rand < 0.7) {
							...
							// maxValue 근처 값 (20% 확률)
							sampleValue = maxValue * 0.9 + Math.random() * maxValue * 0.1;
						} else {
							// maxValue보다 큰 값 (10% 확률) - 테스트용
							sampleValue = maxValue + Math.random() * maxValue * 0.1;
						}
					} else if (item.meta?.minValue) {
						const minValue = item.meta.minValue;
						if (rand < 0.7) {
							// minValue 이상의 안전한 값 (70% 확률)
							sampleValue = minValue + Math.random() * minValue * 0.3;
						} else if (rand < 0.9) {
							// minValue 근처 값 (20% 확률)
							sampleValue = minValue + Math.random() * minValue * 0.1;
						} else {
					...
					const minValue = parseFloat(item.meta?.minValue?.toString() || '0');
					const maxValue = parseFloat(item.meta?.maxValue?.toString() || '0');
				...
				newMeasuredValues[`sample${i}`] = parseFloat(sampleValue.toString());
			}
	...
	const handleFormReady = useCallback((methods: UseFormReturn<Record<string, unknown>>) => {
		...
		methods.watch((value, { name, type }) => {
			...
			// inspectionType 변경 감지
			if (name === 'inspectionType' && type === 'change') {
				const inspectionTypeValue = value.inspectionType;
				if (inspectionTypeValue && typeof inspectionTypeValue === 'string') {
					handleInspectionTypeChange(inspectionTypeValue);
				}
			}
		});
	}, [handleProgressChange, handleInspectionTypeChange]);
	...
	const handleSaveMaster = useCallback(async () => {
		...
		if (selectedProgressId) {
			const progressOption = masterData.progressOptions?.find(option => option.progressId === selectedProgressId);
			if (progressOption) {
		...
		const meta: Meta = {
			...
			progressName: masterData.progressOptions?.find(option => option.progressId === selectedProgressId)?.label || t('tabs.labels.progress'),
		...
		if (missingSampleValues.length > 0) {
			toast.error(`입력되지 않은 시료 값이 있습니다: ${missingSampleValues.length}개`);
			return;
		}
		
		try {
			// 선택된 검사 유형 가져오기
			const selectedInspectionType = formData.inspectionType || masterData.inspectionType || 'SELF';
			...
			const createCheckingHeadPayload: CreateCheckingHeadPayload = {
				...
				targetCode: String(formData.workOrderNo || formData.lotNumber || ''),
				checkingName: `${selectedInspectionType.toUpperCase()}_${String(formData.itemName || 'PRODUCT')}_${masterData.progressOptions?.find(option => option.progressId === selectedProgressId)?.label || 'PROGRESS'}_${new Date().toISOString().split('T')[0]}`,
				isPass: allResultsOK,
				meta: JSON.parse(JSON.stringify(meta))
			};

			const checkingHeadResult = await createCheckingHead.mutateAsync([createCheckingHeadPayload]);
			...
			if (checkingHeadResult && checkingHeadResult.length > 0) {
				...
				inspectionItems.forEach((item) => {
					...
					if (item.specType === 'TOLERANCE' && item.meta?.tolerance !== undefined) {
					...
					for (let i = 1; i <= sampleQuantity; i++) {
						...
						if (sampleValue !== '' && sampleValue !== undefined) {
							createCheckingSamplePayloads.push({
								checkingHeadId,
								sampleIndex: i,
								measuredValue: parseFloat(sampleValue.toString()),
				...
				await createCheckingSample.mutateAsync(createCheckingSamplePayloads);
				
				toast.success(t('inspection.toast.saveInspectionSuccess'));
				
				// 저장 후에도 현재 주소에 따라 다른 페이지로 이동
				const currentPath = location.pathname;
				
				if (currentPath.includes('/production/')) {
					navigate('/production/quality/inspection/list');
				} else {
					navigate('/quality/self-inspection/list');
				}
			}
		} catch (error) {
			...
			}, [inspectionItems, selectedProgressId, createCheckingHead, createCheckingSample, navigate, masterData.progressOptions, masterData.itemId, t, location.pathname]);
	...
	const masterFormFields = useMemo(() => [
		{
			...
			component: (
				<CommandSearchInput
					onCommandFound={handleItemSearch}
					onCommandNotFound={handleDataNotFound}
					placeholder="예: 2501010001"
				/>
			),
			tabIndex: -1,
		},
		{
			...
			component: (
				<LotSearchInput
					onLotFound={handleItemSearch}
					onLotNotFound={handleDataNotFound}
		...
		},
		{
			...
			onChange: (value: string) => {
				if (formMethodsRef.current) {
					formMethodsRef.current.setValue('inspectionType', value);
				}
	...
	], [masterData.progressOptions, handleItemSearch, handleDataNotFound, handleProgressChange, handleInspectionTypeChange, t]);

	// ========================================================================
	// 이펙트
	// ========================================================================
	useEffect(() => {
		if (isInitialized && selectedProgressId) {
			handleProgressChange(selectedProgressId.toString());
		}
	}, [selectedProgressId, handleProgressChange, isInitialized]);

	useEffect(() => {
		if (formMethodsRef.current && isInitialized) {
			const currentFormData = formMethodsRef.current.getValues();
			const currentInspectorName = currentFormData.inspectorName || masterData.inspectorName;
			...
			formMethodsRef.current.reset({
				...
				inspectionType: masterData.inspectionType || 'SELF',
				inspectionDate: getCurrentLocalDateTime(),
				inspectorName: currentInspectorName,
				notes: masterData.notes || ''
			});
		}
	}, [masterData.itemNumber, masterData.itemName, isInitialized, masterData.inspectorName]);
		...
		<div className="max-w-full mx-auto p-4 h-full flex flex-col">
			...
			>
				...
				<div className="border rounded-lg h-full overflow-auto">
					...
					<div className="p-4">
						<DynamicForm
							key="quality-self-inspection-form"
								...
								{inspectionItems.length > 0 && (
									<>
										<button
											...
											title={t('inspection.actions.sampleData')}
											tabIndex={-1}
										>
										...
										<button
											...
											title={t('inspection.actions.clearValues')}
											tabIndex={-1}
										>
											{t('inspection.actions.clearValues')}
										</button>
									</>
								)}
							</div>
...
export default QualitySELFInspectionRegisterPage;