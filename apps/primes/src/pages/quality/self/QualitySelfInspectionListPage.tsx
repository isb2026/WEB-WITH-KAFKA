...
export const QualitySelfInspectionListPage: React.FC = () => {
	...
	// checkingHead 훅 사용 (서버에서 SELF, PATROL 타입만 필터링)
	const { remove, list } = useCheckingHeads({
		page,
		size: PAGE_SIZE,
		searchRequest: {
			...searchParams,
			inspectionTypes: ['SELF', 'PATROL'], // SELF와 PATROL 타입만 서버에서 필터링
		},
	});
	...
	const tableColumns = [
			...
			cell: ({ row }: { row: any }) => {
				...
				const handleCodeClick = () => {
					...
					if (currentPath.includes('/production/')) {
						navigate(`/production/quality/inspection/${row.original.id}`);
					} else {
						navigate(`/quality/self-inspection/report/${row.original.id}`);
					}
				};
		...
		},
		{
			...
			size: 100,
			cell: ({ row }: { row: any }) => {
				const inspectionType = row.original.inspectionType;
				const typeText = inspectionType === 'SELF' 
					? tCommon('inspection.type.self')
					: inspectionType === 'PATROL'
					? tCommon('inspection.type.patrol')
					: inspectionType;
				return typeText;
			},
		},
	...
	const handleEdit = () => {
		if (selectedInspection) {
			...
			if (currentPath.includes('/production/')) {
				...
				navigate(`/quality/self-inspection/edit/${selectedInspection.id}`);
			}
		}
	};
...
export default QualitySelfInspectionListPage;
