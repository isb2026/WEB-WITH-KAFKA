...
const QualityInspectionItemsFinalPage: React.FC = () => {
	...
	const [editingInspection, setEditingInspection] = useState<CheckingSpecData | null>(null);
	...
	const handleEditInspection = () => {
		if (inspectionTable.selectedRows.size === 0) {
			toast.warning(tCommon('inspection.toast.selectItemsToEdit'));
			return;
		}

		const selectedIndex = Array.from(inspectionTable.selectedRows)[0];
	...
	const handleRemoveInspection = async () => {
		try {
			...
			// inspectionTable.selectedRows는 인덱스 기반으로 저장된 값들
			// 인덱스를 실제 데이터의 ID로 변환
			const selectedInspectionIds: number[] = [];
			
			// inspectionData가 있는지 확인
			if (inspectionData) {
				Array.from(inspectionTable.selectedRows).forEach((indexStr) => {
...
export default QualityInspectionItemsFinalPage;
