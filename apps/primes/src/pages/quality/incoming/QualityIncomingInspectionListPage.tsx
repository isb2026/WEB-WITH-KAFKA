...
export const QualityIncomingInspectionListPage: React.FC = () => {
	...
	// checkingHead 훅 사용 (입고검사 타입으로 필터링)
	const { remove, list } = useCheckingHeads({
		page,
		size: PAGE_SIZE,
		searchRequest: {
			...searchParams,
			inspectionType: 'INCOMING', // 입고검사 타입으로 필터링
		},
	});
	...
	const tableColumns = [
		...
		},
		{
			...
			cell: ({ row }: { row: any }) => {
				...
				const handleCodeClick = () => {
					navigate(
						`/quality/incoming-inspection/report/${row.original.id}`
					);
				};
		...
		},
		{
			...
			size: 100,
			cell: tCommon('inspection.type.incoming'),
		},
		{
			id: 'inspectorName',
	...
	const handleEdit = () => {
		if (selectedInspection) {
			navigate(`/quality/incoming-inspection/edit/${selectedInspection.id}`);
		}
	};
		...
		<>
			...
			<PageTemplate>
				<DatatableComponent
					...
					data={data}
					tableTitle={tCommon('inspection.type.incoming')}
...
export default QualityIncomingInspectionListPage;
