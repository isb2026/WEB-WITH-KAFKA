...
const QualityIncomingInspectionRegisterPage: React.FC = (): JSX.Element => {
	...
	const handleItemSearch = useCallback(async (data: any): Promise<void> => {
		...
		try {
			...
			if (progressData.length === 0) {
				toast.warning(t('pages.messages.noProgressForOutsourcingItem'));
			}
	...
	// 공정 선택 시 checkingSpec 데이터로 검사항목 업데이트
	...
	const handleFormReady = useCallback(
		(methods: UseFormReturn<Record<string, unknown>>) => {
			...
			// itemProgress 값 변경 감지 및 폼 데이터 업데이트
	...
	const handleSaveMaster = useCallback(async () => {
		...
		const meta: Meta = {
			itemId: masterData.itemId || 0, // masterData에서 가져온 itemId 사용
		...
		try {
			const createCheckingHeadPayload: CreateCheckingHeadPayload = {
				...
				checkingName: `INCOMING_${String(formData.itemName || 'PRODUCT')}_${masterData.progressOptions?.find((option) => option.progressId === selectedProgressId)?.label || 'PROGRESS'}_${new Date().toISOString().split('T')[0]}`,
				isPass: allResultsOK,
	...
	const handleResetMaster = useCallback(() => {
		...
		setMasterData({
			...
			itemName: '',
			lotNumber: '',
			inspectorName: masterData.inspectorName, // 보존
			inspectionType: INSPECTION_TYPE,
			inspectionDate: getCurrentLocalDateTime(),
			progressOptions: [],
		});
	...
	// selectedProgressId 변경 시 checkingSpecs 조회
	useEffect(() => {
	...
	// masterData 변경 시 폼 강제 업데이트
	useEffect(() => {
		if (formMethodsRef.current && isInitialized) {
			// 현재 폼의 inspectorName과 itemProgress 값을 보존
			...
			formMethodsRef.current.reset({
				...
				itemProgress: currentItemProgress, // 현재 선택된 공정 보존
				inspectionDate: getCurrentLocalDateTime(),
				inspectorName: currentInspectorName, // 현재 입력된 값 보존
		...
		<div className="max-w-full mx-auto p-4 h-full flex flex-col">
			...
			>
				...
				<div className="border rounded-lg h-full overflow-auto">
					...
					<div className="p-4">
						<DynamicForm
							key="quality-incoming-inspection-form"
...
export default QualityIncomingInspectionRegisterPage;
