...
const QualityFinalInspectionRegisterPage: React.FC = (): JSX.Element => {
	// ========================================================================
	// 기본 훅 및 번역
	// ========================================================================
	...
	// 검사 항목 관련 함수
	// ========================================================================
	const handleItemSpecSearch = useCallback(
		async (itemId: number) => {
			if (!itemId) return;
	...
	const handleSaveMaster = useCallback(async () => {
		...
		try {
			const createCheckingHeadPayload: CreateCheckingHeadPayload = {
				...
				checkingName: `FINAL_${String(formData.itemName || 'PRODUCT')}_${new Date().toISOString().split('T')[0]}`,
				isPass: allResultsOK,
			...
			if (checkingHeadResult && checkingHeadResult.length > 0) {
				...
				navigate(`/quality/final-inspection/list`);
			}
		} catch (error) {
		...
		<div className="max-w-full mx-auto p-4 h-full flex flex-col">
			...
			>
				...
				<div className="border rounded-lg h-full overflow-auto">
					...
					<div className="p-4">
						<DynamicForm
							key="quality-final-inspection-form"
...
export default QualityFinalInspectionRegisterPage;
