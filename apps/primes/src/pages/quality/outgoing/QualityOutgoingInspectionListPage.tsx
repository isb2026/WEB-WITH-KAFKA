...
export const QualityOutgoingInspectionListPage: React.FC = () => {
	...
	// checkingHead 훅 사용 (출하검사 타입으로 필터링)
	const { remove, list } = useCheckingHeads({
		page,
		size: PAGE_SIZE,
		searchRequest: {
			...searchParams,
			inspectionType: 'OUTGOING', // 출하검사 타입으로 필터링
		},
	});
	...
	const tableColumns = [
		...
		},
		{
			...
			cell: ({ row }: { row: any }) => {
				...
				const handleCodeClick = () => {
					navigate(
						`/quality/outgoing-inspection/report/${row.original.id}`
					);
				};
		...
		},
		{
			...
			size: 100,
			cell: tCommon('inspection.type.outgoing'),
		},
		{
			id: 'inspectorName',
	...
	const handleEdit = () => {
		if (selectedInspection) {
			navigate(`/quality/outgoing-inspection/edit/${selectedInspection.id}`);
		}
	};
		...
		<>
			...
			<PageTemplate>
				<DatatableComponent
					...
					data={data}
					tableTitle={tCommon('inspection.type.outgoing')}
...
export default QualityOutgoingInspectionListPage;
