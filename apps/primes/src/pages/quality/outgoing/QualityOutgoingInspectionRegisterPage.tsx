...
// 상수 정의
// ============================================================================
const INSPECTION_TYPE = 'OUTGOING';
const DEFAULT_PAGE_SIZE = 10;
const DEFAULT_PAGE = 0;
const MAX_SAMPLE_COUNT = 0;
...
// 메인 컴포넌트
// ============================================================================
const QualityOutgoingInspectionRegisterPage: React.FC = (): JSX.Element => {
	// ========================================================================
	// 기본 훅 및 번역
	// ========================================================================
	...
	// 검사 항목 관련 함수
	// ========================================================================
	const handleItemSpecSearch = useCallback(async (itemId: number) => {
		if (!itemId) return;
		...
		try {
			...
			if (specs.length > 0) {
				const newInspectionItems: InspectionItem[] = specs.map((spec: CheckingSpecData, index: number) => ({
					id: index + 1,
	...
	const handleItemSearch = useCallback(async (data: any): Promise<void> => {
		...
		await handleItemSpecSearch(data.itemId);
	...
	const handleSaveMaster = useCallback(async () => {
		...
		try {
			const createCheckingHeadPayload: CreateCheckingHeadPayload = {
				inspectionType: INSPECTION_TYPE,
				targetId: masterData.itemId || 0,
				targetCode: String(formData.workOrderNo || formData.lotNumber || ''),
				checkingName: `OUTGOING_${String(formData.itemName || 'PRODUCT')}_${new Date().toISOString().split('T')[0]}`,
				isPass: allResultsOK,
			...
			if (checkingHeadResult && checkingHeadResult.length > 0) {
				...
				navigate(`/quality/outgoing-inspection/list`);
			}
		} catch (error) {
			toast.error(t('inspection.toast.saveInspectionError'));
		}
	}, [inspectionItems, createCheckingHead, createCheckingSample, navigate, masterData.itemId, t]);
		...
		const baseColumns = [
			{
				...
				header: t('inspection.inspectionItems.itemName'),
				size: 180,
			},
			{
				accessorKey: 'standardValue',
				header: t('inspection.inspectionItems.standardValue'),
				size: 100,
			},
			{
				accessorKey: 'unit',
			...
			},
			{
				...
				header: tDatatable('columns.upperLimit'),
				size: 80,
				cell: ({ row }: { row: any }) => (
			...
			},
			{
				...
				header: tDatatable('columns.lowerLimit'),
				size: 80,
				cell: ({ row }: { row: any }) => (
	...
	}, [inspectionItems, t, tDatatable, MeasuredValueInput, handleMeasuredValueChange]);
	...
	], [handleItemSearch, handleDataNotFound, t]);
		...
		<div className="max-w-full mx-auto p-4 h-full flex flex-col">
			...
			>
				...
				<div className="border rounded-lg h-full overflow-auto">
					...
					<div className="p-4">
						<DynamicForm
							key="quality-outgoing-inspection-form"
...
export default QualityOutgoingInspectionRegisterPage;