...
export const QualityEquipmentInspectionListPage: React.FC = () => {
	...
	const { remove, list } = useCheckingHeads({
		...
		searchRequest: {
			...searchParams,
			inspectionType: 'MACHINE', // 자주검사 타입으로 필터링
		},
	});
	...
	const tableColumns = [
		...
		},
		{
			...
			header: tDataTable('columns.machineName'),
			size: 150,
			cell: ({ row }: { row: any }) => {
				const meta = row.original.meta;
		...
		},
		{
			...
			cell: ({ row }: { row: any }) => {
				...
				const handleCodeClick = () => {
					navigate(
						`/quality/equipment-daily-check/report/${row.original.id}`
					);
				};
		...
		},
		{
			...
			size: 100,
			cell: tCommon('inspection.type.machine'),
		},
		{
			id: 'inspectorName',
		...
		},
		{
			...
			cell: ({ row }: { row: any }) => {
				...
				// checkingSamples에서 전체 결과 계산
				const totalSamples = checkingSamples?.length || 0;
				const passedSamples = checkingSamples?.filter((sample: any) => sample.isPass === true).length || 0;
				const isPass = totalSamples > 0 && passedSamples === totalSamples;
	...
	const handleEdit = () => {
		if (selectedInspection) {
			navigate(`/quality/equipment-daily-check/edit/${selectedInspection.id}`);
		}
	};
	...
	return (
		<>
			<DeleteConfirmDialog
				...
				title={tCommon('equipment-inspection.delete.title')}
			...
			<PageTemplate>
				<DatatableComponent
					...
					data={data}
					tableTitle={tCommon('inspection.type.machine')}
...
export default QualityEquipmentInspectionListPage;
