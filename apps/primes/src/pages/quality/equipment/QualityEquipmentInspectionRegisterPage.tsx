...
const QualityEquipmentInspectionRegisterPage: React.FC = (): JSX.Element => {
	// ========================================================================
	// 기본 훅 및 번역
	// ========================================================================
	...
	// 설비 정보 조회를 위한 훅
	const { list: machineList } = useMachine({
		page: 0,
		size: 1000, // 모든 설비 정보를 가져오기 위해 큰 값 설정
	});
	...
	// 설비 정보 관련 함수
	// ========================================================================
	const getMachineInfo = useCallback(
		(machineId: number) => {
			if (!machineList.data) return null;

			const machines = Array.isArray(machineList.data)
				? machineList.data
				: machineList.data.content || [];
	...
	// 검사 항목 관련 함수
	// ========================================================================
	const handleMachineSearch = useCallback(
			...
			switch (specType) {
				...
				case 'ONE_SIDED':
					if (item.meta?.minValue !== undefined) {
						result =
							measuredValue >= item.meta.minValue ? 'OK' : 'NG';
					} else if (item.meta?.maxValue !== undefined) {
						result =
							measuredValue <= item.meta.maxValue ? 'OK' : 'NG';
					}
					break;
				...
				case 'RANGE':
					...
					if (rangeMin !== undefined && rangeMax !== undefined) {
						...
						result =
							val >= rangeMin && val <= rangeMax ? 'OK' : 'NG';
					}
					break;
	...
	const masterFormFields = useMemo(
		() => [
			{
				name: 'machineId',
				label: t('inspection.form.machineCodeLabel'),
				type: 'machineName',
				required: true,
				tabIndex: -1,
			},
			...
			{
				name: 'machineName',
				label: t('inspection.form.machineNameLabel'),
				type: 'text',
				disabled: true,
				tabIndex: -1,
			},
			{
				...
				label: t('inspection.form.machineTypeLabel'),
				type: 'text',
				disabled: true,
				tabIndex: -1,
			},
	...
	const handleSaveMaster = useCallback(async () => {
		...
		// 검증
		if (!masterData.machineId) {
			toast.error(t('inspection.toast.machineRequired'));
			return;
		}
		...
		const meta: Meta = {
			machineId: masterData.machineId || 0,
			machineCode: masterData.machineCode || '',
			machineName: masterData.machineName || '',
			machineTypeCode: masterData.machineTypeCode || '',
		...
		try {
			const createCheckingHeadPayload: CreateCheckingHeadPayload = {
				inspectionType: INSPECTION_TYPE,
				targetId: masterData.machineId || 0,
				targetCode: masterData.machineCode || '',
				checkingName: `MACHINE_${String(masterData.machineName || 'EQUIPMENT')}_${new Date().toISOString().split('T')[0]}`,
				isPass: allResultsOK,
			...
			if (checkingHeadResult && checkingHeadResult.length > 0) {
				...
				navigate(`/quality/equipment-daily-check/list`);
			}
		} catch (error) {
		...
		navigate,
		masterData.machineId,
		masterData.machineCode,
		masterData.machineName,
		t,
	]);
	...
	const handleResetMaster = useCallback(() => {
		...
		setCheckingSpecSearchRequest(null);
	}, []);
	...
	return (
		<div className="max-w-full mx-auto p-4 h-full flex flex-col">
			<h1 className="text-xl font-bold mt-2 mb-3">
				{t('inspection.machine.formTitle')}
			</h1>
			...
			>
				...
				<div className="border rounded-lg h-full overflow-auto">
					...
					<div className="p-4">
						<DynamicForm
							key="quality-equipment-inspection-form"
...
export default QualityEquipmentInspectionRegisterPage;
