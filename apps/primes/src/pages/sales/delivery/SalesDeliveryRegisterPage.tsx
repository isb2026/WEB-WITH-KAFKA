...
import { DeliveryDetailRegisterActions } from '@primes/pages/sales/components/DeliveryDetailRegisterActions';
...
export const SalesDeliveryRegisterPage = () => {
	...
	const [newDeliveryMasterId, setNewDeliveryMasterId] = useState<
		number | null
	>(null);
	...
	const { create, listByMasterId, update, remove } = useDeliveryDetail({
		deliveryMasterId: newDeliveryMasterId || 0,
		page: 0,
		size: 30,
	});
	...
	const handleProductFormSubmit = (data: Record<string, unknown>) => {
		...
		// Create payload matching the DeliveryDetailPayload structure
		const payload: CreateDeliveryDetailPayload[] = [{
			deliveryMasterId: newDeliveryMasterId,
	...
	const handleEditFormSubmit = (data: Record<string, unknown>) => {
		...
		// Create payload for updating delivery detail using DeliveryDetailItem type
		const payload: DeliveryDetailItem = {
			deliveryMasterId: newDeliveryMasterId || 0,
			itemId: Number(data.itemId) || 1,
			itemNo: Number(data.itemNo) || 1,
			itemNumber: (data.itemNumber as string) || '품번',
	...
	const populateEditForm = (
		...
		const defaultValues: Record<string, string | number> = {
			...
			deliveryAmount: detail.deliveryAmount?.toString() || '0',
	...
	return (
		<>
			<DraggableDialog
				...
				title={tCommon('pages.delivery.add')}
				content={
					<DynamicForm
			...
			<DraggableDialog
				...
				title={tCommon('pages.delivery.modify')}
				content={
					<DynamicForm
						onFormReady={(methods) => {
			...
			<DeleteConfirmDialog
				...
				title={tCommon('pages.delivery.detailDelete')}
				description={tCommon('pages.delivery.detailDeleteConfirm')}
			/>
			<div className="max-w-full mx-auto p-4 h-full flex flex-col">
				<div className="flex justify-between items-center gap-2 mb-3">
					...
					>
						...
						/>
						{tCommon('pages.delivery.backToDelivery')}
					</RadixButton>
				</div>
				...
				>
					<div className="border rounded-lg overflow-auto h-[calc(100vh-210px)]">
						<SalesDeliveryRegisterForm
							onSuccess={(res: DeliveryMaster) => {
								if (res.id && typeof res.id === 'number') {
									setNewDeliveryMasterId(res.id);
								}
							}}
							onReset={() => {
								setNewDeliveryMasterId(null);
							}}
						/>
					</div>
					{/* h-[calc(100vh-210px)] */}
					<div className="border rounded-lg overflow-hidden">
						<DatatableComponent
							...
							tableTitle={tCommon('pages.delivery.deliveryInfo')}
							rowCount={listByMasterId.data?.totalElements || 0}
							defaultPageSize={30}
							actionButtons={
								<DeliveryDetailRegisterActions
									newDeliveryMasterId={newDeliveryMasterId}
