...
import { TaxInvoiceMasterActions } from '@primes/pages/sales/components/TaxInvoiceMasterActions';
...
export const SalesTaxInvoiceMasterDetailPage = () => {
	const { t } = useTranslation('dataTable');
	const { t: tCommon } = useTranslation('common');

	const SalesTaxInvoiceMasterTableColumns = [
		{
			accessorKey: 'taxDate',
		...
		{
			accessorKey: 'taxInvoiceCode',
			header: t('columns.taxInvoiceCode'),
			size: 140,
		},
	];

	const SalesTaxInvoiceDetailTableColumns = [
		{
	...
	const InfoGridKeys = [
		{ key: 'taxInvoiceCode', label: t('columns.taxInvoiceCode') },
		{ key: 'vendorName', label: t('columns.vendorName') },
		{ key: 'taxDate', label: t('columns.taxDate') },
	...
	const [masterData, setMasterData] = useState<TaxInvoiceMaster[]>([]);
	// ✅ 컴포넌트 마운트 시 선택된 masterId 복원
	const [selectedMasterData, setSelectedMasterData] =
		useState<TaxInvoiceMaster | null>(null);

	// Detail Table State
	const [detailData, setDetailData] = useState<TaxInvoiceDetail[]>([]);
	const [selectedMasterRowId, setSelectedMasterRowId] = useState<number>(0);

	// API hooks
	const { list, remove } = useTaxInvoiceMaster({
		page: masterPage,
		size: PAGE_SIZE,
	});

	// Detail 쿼리 - selectedMasterRowId가 null이 아닐 때만 실행
	const { listByMasterId } = useTaxInvoiceDetail({
		taxInvoiceMasterId: selectedMasterRowId,
		page: 0,
		size: PAGE_SIZE,
	});
	...
	const {
		...
		masterData,
		SalesTaxInvoiceMasterTableColumns,
		PAGE_SIZE,
		masterPageCount,
		masterPage,
		masterTotalElements,
	...
	const {
		...
		detailData,
		SalesTaxInvoiceDetailTableColumns,
		0,
		1,
		0,
		detailData.length,
	...
	useEffect(() => {
		const selectedMasterId = localStorage.getItem('selectedTaxInvoiceMasterId');
		if (selectedMasterId && masterData.length > 0) {
			...
			if (foundMaster) {
				...
				if (rowIndex !== -1) {
					...
					localStorage.setItem('selectedTaxInvoiceMasterId', '0');
				}
			}
		}
	...
	return (
		...
		>
			<div className="border rounded-lg">
				<DatatableComponent
					table={masterTable}
					columns={SalesTaxInvoiceMasterTableColumns}
					data={masterData}
					tableTitle={tCommon('pages.taxInvoice.masterDetail')}
					rowCount={masterTotalElements}
					useSearch={true}
					selectedRows={selectedMasterRows}
					toggleRowSelection={toggleMasterRowSelection}
					actionButtons={
						<TaxInvoiceMasterActions
							selectedRows={selectedMasterRows}
			...
			<div className="h-full">
				...
				>
								...
								>
									<div className="h-full overflow-y-auto">
										{selectedMasterData ? (
											<SalesCommentBlock
												entityId={
													selectedMasterData.taxInvoiceCode ||
											...
											<div className="min-h-[120px] p-4 flex items-center justify-center text-gray-500">
												Please select a tax invoice to
												view comments
											</div>
										)}
									</div>
					...
					<div className="border rounded-lg h-full overflow-hidden">
						<DatatableComponent
							table={detailTable}
							columns={SalesTaxInvoiceDetailTableColumns}
							data={detailData}
							tableTitle={tCommon(
								'pages.taxInvoice.detailDetail'
							)}
...
export default SalesTaxInvoiceMasterDetailPage;
