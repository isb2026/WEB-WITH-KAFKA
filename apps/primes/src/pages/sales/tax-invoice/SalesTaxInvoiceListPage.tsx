...
import { useTaxInvoiceDetailListQuery } from '@primes/hooks/sales/taxInvoiceDetail/useTaxInvoiceDetailListQuery';
import { RadixButton } from '@radix-ui/components';
import { Search, Download } from 'lucide-react';
import { useTranslation } from '@repo/i18n';

export const SalesTaxInvoiceListPage = () => {
	const { t } = useTranslation('dataTable');
	const { t: tCommon } = useTranslation('common');
	const PAGE_SIZE = 30;
	const [page, setPage] = useState(0);
	const [data, setData] = useState<TaxInvoiceDetail[]>([]);
	...
	const SalesTaxInvoiceDetailTableColumns = [
		{
			...
			size: 100,
			cell: ({ row }: any) => {
				const value = row.original.taxInvoiceMaster?.taxInvoiceCode;
				return value ? value : '';
			},
		},
		{
			...
			size: 100,
			cell: ({ row }: any) => {
				const value = row.original.taxInvoiceMaster?.taxDate;
				return value ? value : '';
			},
		},
		{
		...
		},
		{
			...
			size: 100,
			cell: ({ row }: any) => {
				const value = row.original.taxInvoiceMaster?.vendorName;
				return value ? value : '';
			},
		},
	...
	// For tax invoice detail
	const list = useTaxInvoiceDetailListQuery({
		page: page,
		size: PAGE_SIZE,
	});
	...
	return (
		<PageTemplate className="border rounded-lg">
			<DatatableComponent
				...
				data={data}
				tableTitle={tCommon('pages.taxInvoice.list')}
...
export default SalesTaxInvoiceListPage;
