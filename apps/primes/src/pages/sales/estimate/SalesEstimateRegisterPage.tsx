...
import { EstimateDetailRegisterActions } from '@primes/pages/sales/components/EstimateDetailRegisterActions';
...
export const SalesEstimateRegisterPage = () => {
	...
	const [newEstimateMasterId, setNewEstimateMasterId] = useState<
		number | null
	>(null);
	...
	const { create, listByMasterId, update, remove } = useEstimateDetail({
		estimateMasterId: newEstimateMasterId || 0,
		page: 0,
		size: 30,
	});
	...
	const handleProductFormSubmit = (data: Record<string, unknown>) => {
		...
		const payload = [
			{
				estimateMasterId: newEstimateMasterId,
				itemId: 1,
				itemNo: 1,
	...
	return (
		<>
			<DraggableDialog
				...
				title={tCommon('pages.estimate.add')}
				content={
					<DynamicForm
			...
			<DraggableDialog
				...
				title={tCommon('pages.estimate.modify')}
				content={
					<DynamicForm
						onFormReady={(methods) => {
			...
			<DeleteConfirmDialog
				...
				title={tCommon('pages.estimate.detailDelete')}
				description={tCommon('pages.estimate.detailDeleteConfirm')}
			/>
			<div className="max-w-full mx-auto p-4 h-full flex flex-col">
				<div className="flex justify-between items-center gap-2 mb-3">
					...
					>
						...
						/>
						{tCommon('pages.estimate.backToEstimate')}
					</RadixButton>
				</div>
				...
				>
					<div className="border rounded-lg overflow-auto h-[calc(100vh-210px)]">
						<SalesEstimateRegisterForm
							onSuccess={(res: EstimateMaster) => {
								if (res.id && typeof res.id === 'number') {
									setNewEstimateMasterId(res.id);
								}
							}}
							onReset={() => {
								setNewEstimateMasterId(null);
							}}
						/>
					</div>
					{/* h-[calc(100vh-210px)] */}
					<div className="border rounded-lg overflow-hidden">
						<DatatableComponent
							...
							tableTitle={tCommon('pages.estimate.detailInfo')}
							rowCount={listByMasterId.data?.totalElements || 0}
							defaultPageSize={30}
							actionButtons={
								<EstimateDetailRegisterActions
									newEstimateMasterId={newEstimateMasterId}
