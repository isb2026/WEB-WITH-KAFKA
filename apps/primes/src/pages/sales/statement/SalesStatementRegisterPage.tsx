...
import { SalesStatementRegisterForm } from './SalesStatementRegisterForm';
...
import { StatementDetailRegisterActions } from '@primes/pages/sales/components/StatementDetailRegisterActions';
import {
	StatementDetail,
...
import {
	useStatementDetailColumns,
	StatementDataTableDataType,
	statementDetailFormSchema,
	statementDetailEditFormSchema,
} from '@primes/schemas/sales/statementDetailSchemas';
import { useTranslation } from '@repo/i18n';
import { toast } from 'sonner';

export const SalesStatementRegisterPage = () => {
	const { t: tCommon } = useTranslation('common');
	const formMethodsRef = useRef<UseFormReturn<
		Record<string, unknown>
	> | null>(null);
	const statementDetailColumns = useStatementDetailColumns();
	const navigate = useNavigate();
	const processedColumns = useDataTableColumns<StatementDataTableDataType>(
	...
	const [newStatementMasterId, setNewStatementMasterId] = useState<
		number | null
	>(null);
	...
	// Add useStatementDetail hook
	const { create, listByMasterId, update, remove } = useStatementDetail({
		statementMasterId: newStatementMasterId || 0,
		page: 0,
		size: 30,
	});
	...
	useEffect(() => {
		...
		const subscription = formMethods.watch((value, info) => {
			if (info?.name === 'unitPrice' || info?.name === 'netPrice') {
				const rawUnit = value.unitPrice as string;
				const rawNet = value.netPrice as string;
				...
				const formattedNet = commaNumber(net);
				const formattedUnit = commaNumber(unit);

				if (prevGross !== formattedNet) {
					formMethods.setValue('grossPrice', formattedNet);
					formMethods.setValue('unitPrice', formattedUnit);
					formMethods.setValue('netPrice', formattedNet);
				}
			}
		});
	...
	const handleProductFormSubmit = (data: Record<string, unknown>) => {
		if (!newStatementMasterId) {
			console.error('Statement master ID is required');
			return;
		}
		...
		const payload = [
			{
				statementMasterId: newStatementMasterId,
				itemId: 1,
				itemNo: 1,
				...
				statementUnit:
					(data.statementUnit as string)?.substring(0, 3) || 'EA',
				statementNumber: 1, // Default value since we removed it from form
	...
	const handleEditFormSubmit = (data: Record<string, unknown>) => {
		...
		// Create payload for updating statement detail
		const payload = {
			...
			data: {
				...
				itemSpec: (data.itemSpec as string) || '',
				statementUnit:
					(data.statementUnit as string)?.substring(0, 3) || 'KG',
				statementNumber: selectedDetail.statementNumber || 1, // Use existing value
				currencyUnit: (data.currencyUnit as string) || 'KRW',
				unitPrice: Number(uncommaNumber(data.unitPrice as string)) || 0,
				netPrice: Number(uncommaNumber(data.netPrice as string)) || 0,
				grossPrice:
					Number(uncommaNumber(data.grossPrice as string)) || 0,
				memo: selectedDetail.memo || '', // Use existing value
				vat: data.vat ? Number(data.vat) : 0,
			} as Partial<UpdateStatementDetailPayload>,
		};
	...
	const handleEditClick = (detail: StatementDetail) => {
		setSelectedDetail(detail);
		setEditModal(true);
	};

	const handleDeleteClick = (detail: StatementDetail) => {
	...
	const populateEditForm = (
		...
		const defaultValues: Record<string, string | number> = {
			...
			statementUnit: detail.statementUnit || 'EA',
	...
	const processAddRowData = (dataToProcess: Record<string, unknown>[]) => {
		...
		const processedRows = dataToProcess
			.map((rowData: Record<string, unknown>) => {
				...
				const payload = {
					...
					statementUnit: getStringValue('statementUnit', 'EA'),
					statementNumber: getNumberValue('statementNumber', 0),
		...
		create.mutate(payload, {
			...
			onError: (error) => {
				console.error('Error creating statement details:', error);
				toast.error(`명세서 상세 저장 중 오류가 발생했습니다.`);
			},
		});
	};
	...
	return (
		<>
			<DraggableDialog
				...
				title={tCommon('pages.statement.add')}
				content={
					<DynamicForm
						onFormReady={handleFormReady}
						fields={statementDetailFormSchema.map((field) => ({
							...field,
			...
			<DraggableDialog
				...
				title={tCommon('pages.statement.modify')}
				content={
					<DynamicForm
						onFormReady={(methods) => {
						...
						fields={statementDetailEditFormSchema.map((field) => ({
							...field,
			...
			<DeleteConfirmDialog
				...
				title={tCommon('pages.statement.detailDelete')}
				description={tCommon('pages.statement.detailDeleteConfirm')}
			/>
			<div className="max-w-full mx-auto p-4 h-full flex flex-col">
				<div className="flex justify-between items-center gap-2 mb-3">
					...
					>
						...
						/>
						{tCommon('pages.statement.backToStatement')}
					</RadixButton>
				</div>
				...
				>
					<div className="border rounded-lg overflow-auto h-[calc(100vh-210px)]">
						<SalesStatementRegisterForm
							onSuccess={(res: StatementMaster) => {
								if (res.id && typeof res.id === 'number') {
									setNewStatementMasterId(res.id);
								}
							}}
							onReset={() => {
								setNewStatementMasterId(null);
							}}
						/>
					</div>
					{/* h-[calc(100vh-210px)] */}
					<div className="border rounded-lg overflow-hidden">
						<DatatableComponent
							...
							tableTitle={tCommon('pages.statement.detailInfo')}
							rowCount={listByMasterId.data?.totalElements || 0}
							defaultPageSize={30}
							actionButtons={
								<StatementDetailRegisterActions
									newStatementMasterId={newStatementMasterId}
