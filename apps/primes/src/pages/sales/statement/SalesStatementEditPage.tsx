...
import { useGetStatementMasterById } from '@primes/hooks/sales/statementMaster/useGetStatementMasterById';
import { DeleteConfirmDialog } from '@primes/components/common/DeleteConfirmDialog';
import { StatementDetailRegisterActions } from '../components/StatementDetailRegisterActions';
...
export const SalesStatementEditPage = () => {
	...
	const statementMasterId = parseInt(id || '0');

	// Add useGetStatementMasterById hook to get master data by ID
	const getById = useGetStatementMasterById(statementMasterId, 0, 30);
	...
	const {
		...
		statementMasterId: statementMasterId,
		page: 0,
		size: 30,
	});
	...
	useEffect(() => {
		if (statementMasterId > 0) {
			// The query will automatically run when statementMasterId changes
		}
	}, [statementMasterId]);
	...
	useEffect(() => {
		...
		const subscription = formMethods.watch((value, info) => {
			if (
				info?.name === 'unitPrice' ||
				info?.name === 'statementNumber'
			) {
				const rawUnit = value.unitPrice as string;
				const rawNumber = value.statementNumber as string;
		...
		const payload = [
			{
				...
				statementNumber:
					Number(uncommaNumber(data.statementNumber as string)) || 1,
	...
	const handleEditFormSubmit = (data: Record<string, unknown>) => {
		...
		const payload = {
			...
			data: {
				statementMasterId: statementMasterId || 0,
				itemId: 1,
				itemNo: 1,
				itemNumber: data.itemNumber as string,
				itemName: data.itemName as string,
	...
	const populateEditForm = (
		...
		const defaultValues: Record<string, string | number> = {
			...
			statementNumber: detail.statementNumber?.toString() || '0',
			statementUnit: detail.statementUnit || '개',
	...
	return (
		<>
			...
			<DraggableDialog
				...
				title={t('pages.statement.modify')}
				content={
					<DynamicForm
						onFormReady={(methods) => {
			...
			<DeleteConfirmDialog
				...
				title={t('pages.statement.detailDelete')}
				description={t('pages.statement.detailDeleteConfirm')}
			/>
			<div className="max-w-full mx-auto p-4 h-full flex flex-col">
				<div className="flex justify-between items-center gap-2 mb-3">
					...
					>
						...
						/>
						{t('pages.statement.backToStatement')}
					</RadixButton>
				</div>
				...
				>
					...
					<div className="border rounded-lg overflow-hidden">
						<DatatableComponent
							...
							tableTitle={t('pages.statement.detailInfo')}
							rowCount={listByMasterId.data?.totalElements || 0}
							defaultPageSize={30}
							actionButtons={
								<StatementDetailRegisterActions
									newStatementMasterId={statementMasterId}
...
export default SalesStatementEditPage;
