...
import { OrderDetailRegisterActions } from '@primes/pages/sales/components/OrderDetailRegisterActions';
...
export const SalesOrderRegisterPage = () => {
	...
	const processedColumns =
		useDataTableColumns<OrderDetail>(orderDetailColumns);
	...
	const { createDetail, listByMasterId, updateDetail, removeDetail } =
		useOrderDetail({
			orderMasterId: newOrderMasterId || 0,
			page: 0,
			size: 30,
		});
	...
	const handleFormReady = (
		...
		const subscription = methods.watch((value, { name }) => {
			if (name === 'itemSelect' && value.itemSelect) {
				...
				const selectedItem = list.data?.content?.find(
					(item: ItemDto) =>
	...
	useEffect(() => {
		...
		const subscription = formMethods.watch((value, info) => {
			if (info?.name === 'unitPrice' || info?.name === 'orderNumber') {
				...
				const unitPrice =
					Number(uncommaNumber(rawUnitPrice ?? '')) || 0;
				const orderNumber =
					Number(uncommaNumber(rawOrderNumber ?? '')) || 0;
				...
				if (
					prevNetPrice !== formattedNetPrice ||
					prevGrossPrice !== formattedGrossPrice
				) {
	...
	const handleProductFormSubmit = (data: Record<string, unknown>) => {
		...
		const payload = [
			{
				orderMasterId: newOrderMasterId,
				itemId: 1,
				itemNo: 1,
				itemNumber: (data.itemNumber as string) || '',
				itemName: data.itemName as string,
				orderUnit: data.orderUnit as string,
				orderNumber: Number(uncommaNumber(data.orderNumber as string)),
	...
	const handleEditFormSubmit = (data: Record<string, unknown>) => {
		...
		const payload: OrderDetailItem = {
			orderMasterId: newOrderMasterId || 13,
			itemId: 1,
			itemNo: 1,
	...
	const populateEditForm = (
		...
		const defaultValues: Record<string, string | number> = {
			...
			orderUnit: detail.orderUnit || '개', // Default value if not present
	...
	return (
		<>
			...
			<DraggableDialog
				...
				title={tCommon('pages.order.modify')}
				content={
					<DynamicForm
						onFormReady={(methods) => {
			...
			<DeleteConfirmDialog
				...
				title={tCommon('pages.order.detailDelete')}
				description={tCommon('pages.order.detailDeleteConfirm')}
			/>
			<div className="max-w-full mx-auto p-4 h-full flex flex-col">
				<div className="flex justify-between items-center gap-2 mb-3">
					...
					>
						...
						/>
						{tCommon('pages.order.backToOrder')}
					</RadixButton>
				</div>
				...
				>
					...
					<div className="border rounded-lg overflow-hidden flex-1 h-full">
						<DatatableComponent
							...
							tableTitle={tCommon('pages.order.orderInfo')}
							rowCount={listByMasterId.data?.totalElements || 0}
							defaultPageSize={30}
							actionButtons={
								<OrderDetailRegisterActions
									newOrderMasterId={newOrderMasterId}
							...
							defaultRowValues={{
								requestDate:
									currentRequestDate || getTodayDateString(),
							}}
							onAddRow={(newRow) => {
