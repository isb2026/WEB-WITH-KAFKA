...
import { useShipmentDetailListQuery } from '@primes/hooks/sales/shipmentDetail/useShipmentDetailListQuery';
import { RadixButton } from '@radix-ui/components';
import { Search, Download } from 'lucide-react';
import { useTranslation } from '@repo/i18n';

export const SalesShipmentListPage = () => {
	...
	const SalesShipmentDetailTableColumns = [
		{
			...
			size: 120,
			cell: ({ row }: any) => {
				const value = row.original.shipmentMaster?.shipmentCode;
				return value ? value : '';
			},
		},
		{
			...
			size: 120,
			cell: ({ row }: any) => {
				const value = row.original.shipmentMaster?.requestDate;
				return value ? value : '';
			},
		},
		{
			...
			size: 100,
			cell: ({ row }: any) => {
				const value = row.original.shipmentMaster?.vendorName;
				return value ? value : '';
			},
		},
		{
	...
	// For shipment detail
	const list = useShipmentDetailListQuery({
		page: page,
		size: PAGE_SIZE,
	});
	...
	return (
		<PageTemplate className="border rounded-lg">
			<DatatableComponent
				...
				data={data}
				tableTitle={tCommon('pages.shipment.list')}
...
export default SalesShipmentListPage;
