...
import { ShippingRequestDetailRegisterActions } from '@primes/pages/sales/components/ShippingRequestDetailRegisterActions';
import { toast } from 'sonner';

export const SalesShippingRequestRegisterPage = () => {
	...
	const [newShippingRequestMasterId, setNewShippingRequestMasterId] =
	...
	const { create, listByMasterId, update, remove } = useShippingRequestDetail(
		{
			masterId: newShippingRequestMasterId || 0,
			page: 0,
			size: 30,
		}
	);
	...
	const handleDetailFormSubmit = (data: Record<string, unknown>) => {
		...
		const payload = [
			{
				shippingRequestMasterId: newShippingRequestMasterId,
				itemId: 1,
				itemNo: 1,
		...
		const payload = [
			{
				...
				shippingRequestMasterId: newShippingRequestMasterId || 0,
				itemId: 1,
				itemNo: 1,
				itemNumber: (data.itemNumber as string) || '',
				itemName: (data.itemName as string) || '',
	...
	return (
		<>
			<DraggableDialog
				...
				title={tCommon('pages.shippingRequest.add')}
				content={
					<DynamicForm
			...
			<DraggableDialog
				...
				title={tCommon('pages.shippingRequest.modify')}
				content={
					<DynamicForm
						onFormReady={(methods) => {
			...
			<DeleteConfirmDialog
				...
				title={tCommon('pages.shippingRequest.detailDelete')}
				description={tCommon(
					'pages.shippingRequest.detailDeleteConfirm'
				)}
			/>
			<div className="max-w-full mx-auto p-4 h-full flex flex-col">
				<div className="flex justify-between items-center gap-2 mb-3">
					...
					>
						...
						/>
						{tCommon('pages.shippingRequest.backToShippingRequest')}
					</RadixButton>
				</div>
				...
				>
					<div className="border rounded-lg overflow-auto h-[calc(100vh-210px)]">
						<SalesShippingRequestRegisterForm
							onSuccess={(res: ShippingRequestMaster) => {
								if (res.id && typeof res.id === 'number') {
									setNewShippingRequestMasterId(res.id);
								}
							}}
							onReset={() => {
								setNewShippingRequestMasterId(null);
							}}
						/>
					</div>
					<div className="border rounded-lg overflow-hidden">
						<DatatableComponent
							...
							tableTitle={tCommon(
								'pages.shippingRequest.detailInfo'
							)}
...
export default SalesShippingRequestRegisterPage;
