...
import { ShippingRequestMasterActions } from '@primes/pages/sales/components/ShippingRequestMasterActions';
...
export const SalesShippingRequestMasterDetailPage: React.FC = () => {
	const { t } = useTranslation('dataTable');
	const { t: tCommon } = useTranslation('common');

	const SalesShippingRequestMasterTableColumns = [
		{
	...
	const InfoGridKeys = [
		{ key: 'shippingRequestCode', label: t('columns.shippingRequestCode') },
	...
	const [masterData, setMasterData] = useState<ShippingRequestMaster[]>([]);
	const [selectedMasterData, setSelectedMasterData] =
		useState<ShippingRequestMaster | null>(null);

	// Detail Table State
	const [detailData, setDetailData] = useState<ShippingRequestDetail[]>([]);
	const [selectedMasterRowId, setSelectedMasterRowId] = useState<number>(0);

	// API hooks
	const { list, remove } = useShippingRequestMaster({
		page: masterPage,
		size: PAGE_SIZE,
	});

	const { listByMasterId } = useShippingRequestDetail({
	...
	const {
		...
		masterData,
		SalesShippingRequestMasterTableColumns,
		PAGE_SIZE,
		masterPageCount,
		masterPage,
		masterTotalElements,
	...
	useEffect(() => {
		const selectedMasterId = localStorage.getItem('selectedShippingRequestMasterId');
		if (selectedMasterId && masterData.length > 0) {
			...
			if (foundMaster) {
				...
				if (rowIndex !== -1) {
					...
					localStorage.setItem('selectedShippingRequestMasterId', '0');
				}
			}
		}
	...
	return (
		<PageTemplate
			firstChildWidth="30%"
			splitterSizes={[30, 70]}
			splitterMinSize={[450, 800]}
			splitterGutterSize={6}
		>
			<div className="border rounded-lg">
				<DatatableComponent
					table={masterTable}
					columns={SalesShippingRequestMasterTableColumns}
					data={masterData}
					tableTitle={tCommon('pages.shippingRequest.masterDetail')}
				...
				>
					...
					<div className="border rounded-lg h-full overflow-hidden">
						<DatatableComponent
							...
							data={detailData}
							tableTitle={tCommon(
								'pages.shippingRequest.detailDetail'
							)}
...
export default SalesShippingRequestMasterDetailPage;
