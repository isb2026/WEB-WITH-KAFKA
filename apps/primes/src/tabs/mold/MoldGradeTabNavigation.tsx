...
const MoldGradeTabNavigation: React.FC<TabNavigationProps> = ({
	activetab,
}) => {
	...
	useEffect(() => {
		...
		if (pathname.includes('/mold/grade/list')) {
			setCurrentTab('list');
		}
	}, [location.pathname]);
	...
	return (
		<>
			<DraggableDialog
				...
				title={tCommon('pages.mold.grade.register')}
				content={
			...
			/>
			<TabLayout
				title={tCommon('pages.mold.grade.management')}
				tabs={tabs}
...
export default MoldGradeTabNavigation;
