...
const MoldLifeChangeHistoryTabNavigation: React.FC<TabNavigationProps> = ({
	activetab,
}) => {
	...
	useEffect(() => {
		...
		if (pathname.includes('/mold/life/list')) {
			setCurrentTab('list');
		}
	}, [location.pathname]);
	...
	return (
		<>
			<DraggableDialog
				...
				title={tCommon('pages.mold.lifeChangeHistory.register')}
				content={
			...
			/>
			<TabLayout
				title={tCommon('pages.mold.lifeChangeHistory.management')}
				tabs={tabs}
...
export default MoldLifeChangeHistoryTabNavigation;
