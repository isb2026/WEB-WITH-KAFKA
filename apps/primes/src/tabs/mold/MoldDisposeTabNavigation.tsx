...
const MoldDisposeTabNavigation: React.FC<TabNavigationProps> = ({
	activetab,
}) => {
	...
	useEffect(() => {
		...
		if (pathname.includes('/mold/dispose/list')) {
			setCurrentTab('list');
		}
	}, [location.pathname]);
	...
	return (
		<>
			<DraggableDialog
				...
				title={tCommon('pages.mold.dispose.register')}
				content={
			...
			/>
			<TabLayout
				title={tCommon('pages.mold.dispose.management')}
				tabs={tabs}
...
export default MoldDisposeTabNavigation;
