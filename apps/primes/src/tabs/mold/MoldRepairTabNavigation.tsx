...
const MoldRepairTabNavigation: React.FC<TabNavigationProps> = ({
	activetab,
}) => {
	...
	useEffect(() => {
		...
		if (pathname.includes('/mold/repair/list')) {
			setCurrentTab('list');
		}
	}, [location.pathname]);
	...
	return (
		<>
			<DraggableDialog
				...
				title={tCommon('pages.mold.repair.register')}
				content={
			...
			/>
			<TabLayout
				title={tCommon('pages.mold.repair.management')}
				tabs={tabs}
...
export default MoldRepairTabNavigation;
