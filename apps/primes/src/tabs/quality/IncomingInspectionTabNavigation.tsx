...
import {
	QualityIncomingAnalysisPage,
	QualityIncomingInspectionListPage,
...
const IncomingInspectionTabNavigation: React.FC<TabNavigationProps> = ({
	activetab,
}) => {
	...
	useEffect(() => {
		...
		if (pathname.includes('/quality/incoming-inspection/list')) {
			setCurrentTab('list');
		} else if (pathname.includes('/quality/incoming-inspection/report')) {
	...
	const tabs: TabItem[] = [
		{
			...
			content: <QualityIncomingInspectionListPage />,
		},
		{
			id: 'analysis',
			...
			content: <QualityIncomingAnalysisPage />,
		},
	];
	...
	const RegisterButton = () => (
		<div className="ml-auto shrink-0 flex-grow-0 flex gap-2">
			<RadixIconButton
				...
				onClick={() =>
					navigate('/quality/incoming-inspection/register')
				}
			>
				<Plus size={16} />
	...
	return (
		<TabLayout
			title={t('tabs.titles.incomingInspection')}
			tabs={tabs}
...
export default IncomingInspectionTabNavigation;
