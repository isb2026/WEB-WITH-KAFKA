...
export const uncommaNumber = (str) => {
	if (typeof str === 'number') return str; // 이미 숫자면 그대로 반환
	if (typeof str !== 'string') return NaN; // 예외 처리

	const cleaned = str.replace(/,/g, '');
	return isNaN(cleaned) ? NaN : Number(cleaned);
};
